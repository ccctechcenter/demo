...
--   curr_release            CONSTANT VARCHAR2 (30)             := '8.7.1.1[C3SC 8.11.0.1]';
   --LOCAL02 sig
--added following variables for use with course shopping cart
        term2use ssbsect.ssbsect_term_code%Type;
        newrsts  stvrsts.stvrsts_code%Type;
--end local02
--local01 change release number
...
--local02 mods  ??? not sure about this one 
         Or
          --                                                 
          --local02 BUTTON from shopping cart. Could contain crns to add to registration
-- ??? This was 0280 in Banner 7
           ltrim(rtrim(lv_literal(2))) = g$_nls.get('BWCKCOM1-0012',
                                                 'SQL',
                                                 'Register Selections')
--end local02                                                                                   
      THEN
...
--                           commit_flag := 'N';
         ...
         END;
      END LOOP;
--LOCAL02 delete shopping cart records
        Begin
            Delete From szrcart a
            Where  a.pidm = genpidm
            And    a.term = twbkwbis.f_getparam(genpidm,
                                                'TERM');
            Commit;
        End;
--end local02                    
...
-- ==================================================
         ...
         twbkfrmt.p_tabledataheader (
            twbkfrmt.f_printtext (
               '<ABBR title = ' || g$_nls.get ('BWCKCOM1-0050',
                                      'SQL',
                                      'Section') ||
                  '>' ||
                  g$_nls.get ('BWCKCOM1-0051', 'SQL', 'Sec') ||
                  '</ABBR>'
            )
         );
         twbkfrmt.p_tabledataheader (g$_nls.get ('BWCKCOM1-0052', 'SQL', 'Level'));
         twbkfrmt.p_tabledataheader (
            twbkfrmt.f_printtext (
               '<ABBR title = "' ||
...
-- ==================================================
   ...
   IS
    ...
    --LOCAL15 start
    subject_code     ssbsect.ssbsect_subj_code%TYPE;
    course_number    ssbsect.ssbsect_crse_numb%TYPE;
    drop_warning1_in varchar2(500);
  
    cursor c_drop_message1 is
      select subject_code, course_number, crn
        from sv_student_term_repeats
       where pidm = global_pidm
         and term_code = term
         and prior_number_enrolled = 1;
  
    cursor c_drop_message2 is
      select subject_code, course_number, crn
        from sv_student_term_repeats
       where pidm = global_pidm
         and term_code = term
         and prior_number_enrolled = 2;
    --LOCAL15 end			
   ...
   BEGIN
    ...
    --LOCAL15 start
      ...
      --message for 1st repeat
      for x in c_drop_message1 loop
        twbkfrmt.P_PrintMessage('<font color=red>* Repeat Warning:</font> You have already taken ' ||
                                x.subject_code || ' ' || x.course_number ||
                                ' (CRN ' || x.crn ||
                                ') once.  Only 3 attempts to pass a class are allowed.  Please consult the current catalog for information about repeat limitations.');
        twbkfrmt.P_Paragraph(1);
      end loop;
    
      --message for 2nd repeat
      for y in c_drop_message2 loop
        twbkfrmt.P_PrintMessage('<font color=red>* Repeat Warning:</font> You have already taken ' ||
                                y.subject_code || ' ' || y.course_number ||
                                ' (CRN ' || y.crn ||
                                ') two times, this will be your third and final attempt.  Please see a counselor to enhance your chances of successfully completing the class.');
        twbkfrmt.P_Paragraph(1);
      end loop;
      ...
      --LOCAL15 end				 
         ...
         twbkfrmt.p_tablerowopen;
         twbkfrmt.p_tabledata (g$_nls.get ('BWCKCOM1-0065', 'SQL', 'Date') || ':');
         twbkfrmt.p_tabledata (
            TO_CHAR (
               SYSDATE,
               twbklibs.date_display_fmt || ' ' ||
...
-- =====================================================================
   ...
   BEGIN
      ...
      twbkfrmt.p_tabledataheader (
         twbkfrmt.f_printtext (
            '<ABBR title = ' || g$_nls.get ('BWCKCOM1-0077', 'SQL', 'Section') ||
               '>' ||
               g$_nls.get ('BWCKCOM1-0078', 'SQL', 'Sec') ||
               '</ABBR>'
         )
      );
      twbkfrmt.p_tabledataheader (g$_nls.get ('BWCKCOM1-0079', 'SQL', 'Level'));
      twbkfrmt.p_tabledataheader (
         twbkfrmt.f_printtext (
            '<ABBR title = "' ||
...
-- ===================================================
      ...
      twbkfrmt.p_printheader (
         '3',
         g$_nls.get ('BWCKCOM1-0085', 'SQL', 'Add Classes Worksheet')
      );
...
-- =====================================================================
-- This procedure displays the new crn entry boxes on the add/drop page.
-- WFP local02,05,11,12 - replaced everything in baseline from the first
--     local05 reference below down to the end of the procedure.
-- =====================================================================
...
--local05
        ...
        htp.print('<tr><td width=55% valign="center" align="top">');
--added new text for submit buttons        
--local11
        htp.print('<div id="HighlightBox">');
        htp.formsubmit('REG_BTN',
                       g$_nls.get('BWCKCOM1-0342',
                                  'SQL',
                                  'Save Schedule Changes'));
-- local12   old line that was changed below
        htp.print('<p class="infotext" style="margin-top:0px;">
            By clicking this button, you acknowledge that you will be registered in all available classes and responsible for all fees due, including the remote registration fee.
            </p></div>');      
--end local11
/* local14
        htp.formreset(g$_nls.get('BWCKCOM1-0278',
                                 'SQL',
                                 'Reset Form'));                       
        htp.formclose;
        htp.formopen(twbkwbis.f_cgibin || 'bwskffee.P_FeeAsses',
                     cattributes => 'onSubmit="return checkSubmit()"');    
        htp.formsubmit(Null,
                       'Complete Registration & Pay Fees');

*/ 
htp.print('</td></tr></tbody></table>');
        htp.formclose; 
        -----------------------------------------------------------------------------------
--        htp.print('</td><td width=45% valign="center" align="top" ><center>');
htp.print('<table><tr><td><center>');

--LOCAL02 tjl Shopping cart addition to replace course search.  
--
--Added link to optional fees. tjl           
        htp.print('<table width=100% border=0><tr><td width=50% align="top" valign="center">');
        term2use := twbkwbis.f_getparam(global_pidm,
                                        'TERM');
-- ??? Needed?
        pw_register.p_calc_primary_camp(global_pidm,
                             term2use);
        htp.formopen(twbkwbis.f_cgibin || 'pw_crse_search.p_CrseSearch_Simple',
                     cattributes => 'onSubmit="return checkSubmit()"');
        htp.formhidden('TERM_IN',
                       term2use);
        htp.formhidden('PIDM_IN',
                       global_pidm);
        htp.formsubmit(Null,
                       'Course Shopping Cart');
        htp.formclose;
        --mod for optional fees            
        htp.print('</td><td align="top" width=50%>');
        --mod for optional fees
        htp.formopen(twbkwbis.f_cgibin || 'pw_web_fees.p_feesgate',
                     '_parent',
                     Null,
                     Null,
                     Null);
        ...
        htp.formhidden('TERM_IN',
                       term2use);
        htp.formhidden('PROC_IN',
                       'bwskfreg.p_Altpin');
        htp.formsubmit(Null,
                       'Purchase Student Photo ID');
        htp.p('</form>');
        htp.print('</td></tr></table></table>');
        
        --local14
        htp.print('<div style="text-align:center;margin:20px;">');
        htp.formopen(twbkwbis.f_cgibin || 'bwskffee.P_FeeAsses',
                     cattributes => 'onSubmit="return checkSubmit()"');        
        htp.formsubmit(Null,
        ...
        htp.formclose;
        htp.print('<span class="infotext"><img src="../wtlgifs/web_caution_cascade.png" width="17" height="15" /> Registration is not finished until you click this button and pay fees.</span></div>');

/*                                 
      HTP.formsubmit (
         'REG_BTN',
         g$_nls.get ('BWCKCOM1-0091', 'SQL', 'Submit Changes')
      );
      ...
      HTP.formreset (g$_nls.get ('BWCKCOM1-0093', 'SQL', 'Reset'));
