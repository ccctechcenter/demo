...
-- SOURCE  : enUS
-- TARGET  : I18N
-- DATE    : Tue May 05 16:21:11 2015
-- MSGSIGN : #d3152be8db87ea57
...
-- COPYRIGHT..: Copyright 2006 - 2015 Ellucian Company L.P. and its affiliates.
...
/* Update current version */
   curr_release   CONSTANT VARCHAR2 (30)             := '8.11A C3SC';
   ...
   --local01 mcm change button text
   --cancel_btn     CONSTANT VARCHAR2(20):= g$_nls.get('BWVKAUT1-0000','SQL','Cancel');
   cancel_btn     CONSTANT VARCHAR2(20):= g$_nls.get('X','SQL','Reset');
   subm_btn       CONSTANT VARCHAR2(50):= g$_nls.get('BWVKAUT1-0001','SQL','Submit Changes');
   --local01 mcm change button text
   --validate_btn   CONSTANT VARCHAR2(50):= g$_nls.get('BWVKAUT1-0002','SQL','Validate');
   validate_btn   CONSTANT VARCHAR2(50):= g$_nls.get('X','SQL','Submit');
...
-- ==================================================
  ...
  twbkfrmt.p_tableopen ('DATAENTRY',
    ...
    twbkfrmt.p_tablerowopen;
      twbkfrmt.p_tabledataheader (g$_nls.get ('BWVKAUT1-0004', 'SQL', 'Status'));
      twbkfrmt.p_tabledataheader (g$_nls.get ('BWVKAUT1-0005', 'SQL', 'Registration Add Auth Code'));
      twbkfrmt.p_tabledataheader (twbkfrmt.f_printtext ('<ACRONYM title = "' ||
                                                        g$_nls.get ('BWVKAUT1-0006','SQL','Course Reference Number') ||
                                                        '">' ||
                                                        g$_nls.get ('BWVKAUT1-0007', 'SQL', 'CRN') ||
                                                        '</ACRONYM>'
                                                       )
                                  );
      twbkfrmt.p_tabledataheader (twbkfrmt.f_printtext ('<ABBR title = ' ||
                                                        g$_nls.get ('BWVKAUT1-0008','SQL','Subject') ||
                                                        '>' ||
                                                        g$_nls.get ('BWVKAUT1-0009', 'SQL', 'Subj') ||
                                                       '</ABBR>'
                                                        )
                                    );
      twbkfrmt.p_tabledataheader (twbkfrmt.f_printtext ('<ABBR title = ' ||
                                                        g$_nls.get ('BWVKAUT1-0010', 'SQL', 'Course') ||
                                                        '>' ||
                                                        g$_nls.get ('BWVKAUT1-0011', 'SQL', 'Crse') ||
                                                        '</ABBR>'
                                                         )
                                   );
      twbkfrmt.p_tabledataheader (twbkfrmt.f_printtext ('<ABBR title = ' ||
                                                         g$_nls.get ('BWVKAUT1-0012','SQL','Section') ||
                                                         '>' ||
                                                         g$_nls.get ('BWVKAUT1-0013', 'SQL', 'Sec') ||
                                                        '</ABBR>'
                                                        )
                                  );
      twbkfrmt.p_tabledataheader (twbkfrmt.f_printtext ('<ABBR title = "' ||
                                                        g$_nls.get ('BWVKAUT1-0014', 'SQL', 'Credit Hours') ||
                                                        '">' ||
                                                        g$_nls.get ('BWVKAUT1-0015', 'SQL', 'Cred') ||
                                                        '</ABBR>'
                                                          )
                                    );
      twbkfrmt.p_tabledataheader (g$_nls.get ('BWVKAUT1-0016', 'SQL', 'Title'));
      twbkfrmt.p_tabledataheader (g$_nls.get ('BWVKAUT1-0017', 'SQL', 'Reason'));
    ...
    --Internal Defect:1721  STU16:GRS 04/04/2006
         ...
         END;  
          ...
          ELSIF temp_num_attempts <= 3 THEN
            ...
            twbkfrmt.p_formlabel (g$_nls.get ('BWVKAUT1-0018', 'SQL', 'Registration Add Auth Code'),
                                  visible   => 'INVISIBLE',
...
-- ==================================================
         ...
         IF temp_num_attempts >3 THEN
            twbkfrmt.p_tabledata(g$_nls.get ('BWVKAUT1-0019', 'SQL','MAXIMUM ATTEMPTS REACHED'));  	
         ELSE
...
-- local01 change button text
--       sub_btn_in              IN   VARCHAR2   DEFAULT null ,    --c3sc 8.10 grs 05/05/2015 changed
       sub_btn_in              IN   VARCHAR2   DEFAULT g$_nls.get('X','SQL','Submit') , 
       capp_tech_error_in_out  IN OUT  VARCHAR2,
       drop_result_label_in    IN twgrinfo.twgrinfo_label%TYPE DEFAULT NULL,
...
-- sftregs_rec                       sftregs%ROWTYPE;             /*C3SC 8.7.1.1 GRS 04/OCT/2013 MODIFIED */
 ...
 BEGIN
  ...
  THEN
    RETURN;
  END IF;	
  -- C3SC 8.10  GRS 05/05/2015 begin
  IF sub_btn_in IS NULL THEN
    sub_btn:= g$_nls.get('BWVKAUT1-0020','SQL','Validate');
  ELSE
    sub_btn:=sub_btn_in;
  END IF;
 -- C3SC 8.10  GRS 05/05/2015 end 
-- Defect 1694
...
-- end Defect   
  ...
  END IF;
  --
  --C3SC 8.9.1.1 GRS 06/18/2014 ADDED
  IF sub_btn <> cancel_btn THEN
    called_by_proc_name := 'bwckcoms.P_Regs';
    FOR i in 2..orig_crn.COUNT
    LOOP
      IF LTRIM (RTRIM (orig_rsts (i))) IS NOT NULL
       THEN
         IF NOT bwcklibs.f_is_crs_reg_status_valid( orig_rsts (i), term, orig_crn (i) ) THEN
		...
		ELSE
		  ...
		  IF sv_acad_calendar_bp.F_Calc_Drop_is_Active(p_term_code=>term) = 'Y' THEN
			IF NOT sv_acad_calendar_bp.f_is_crs_calc_drop_valid(p_pidm_in=>global_pidm,
					                                            p_rsts_code_in=>orig_rsts(i),
															    p_term_code_in=>term,
															    p_crn_code_in=>orig_crn (i) 
																) THEN
      ...
      END IF;  
     END LOOP;
    --C3SC 8.9.1.1 GRS 06/18/2014 END
   END IF;	
...
ELSIF sub_btn = subm_btn THEN
       ...
       IF auth_msg_code(i)='AUTH1' THEN
        ...
        BEGIN
		 ...
		 sftregs_rec.sftregs_error_flag := 'O';    -- C3SC 8.11 CR-000136237  GRS 11/23/2015
...
-- ===================================================
          ...
          sv_reg_inprogress.p_update(p_term_code  => sftregs_rec.sftregs_term_code,
                                     ...
                                     p_error_flag => sftregs_rec.sftregs_error_flag,     -- C3SC 8.11 CR-000136237  GRS 11/23/2015
  ...
  /* CALB 8.7 04/16/2013 ADDED AFTER THE AUTH REQUIRED MESSAGE IS REMOVED WE STILL NEED TO CHECK FOR REPEATING (ITS NEEDED FOR FAMILY REPEAT)*/
	    ...
	    p_getoverride;	   /*C3SC 8.7.1.1 GRS 04/OCT/2013 ADDED */
		 stcr_err_ind:= NULL;   -- C3SC 8.11 CR-000136237  GRS 11/23/2015
         appr_err := NULL;      -- C3SC 8.11 CR-000136237  GRS 11/23/2015
