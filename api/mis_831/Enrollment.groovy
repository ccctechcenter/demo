 ...
 *    BWCKREGS: 8.7.1.1[C3SC 8.11.0.1]
 *    BWCKCOMS: 8.7.1.1[C3SC 8.11.0.1]
 *    BWCKSAMS: 8.7.1.1[C3SC 8.11.0.1]
 *    BWSKFREG: 8.7.1
 *    BWVKAUTH: 8.11 C3SC
        ...
        --
              ...
              ELSE
                row_count := 0;
                 ...
                 LOOP
                   IF     sftregs_row.sftregs_levl_code = scrlevl.scrlevl_levl_code
                    THEN
                       row_count := scklibs.scrlevlc%rowcount;
                       EXIT;
                    END IF;
...
-- <<Coast>> -- start changes TRvvvv 9/30/2015
   part_of_term_description     sobptrm.sobptrm_desc%TYPE       := NULL;
   part_of_term_start           sobptrm.sobptrm_start_date%TYPE := NULL;
   part_of_term_end             sobptrm.sobptrm_end_date%TYPE   := NULL;

   CURSOR part_of_term_desc_c (term_in sobptrm.sobptrm_term_code%TYPE,
            ptrm_in sobptrm.sobptrm_ptrm_code%TYPE)
      IS
         SELECT SOBPTRM_DESC
                ,SOBPTRM_START_DATE
                ,SOBPTRM_END_DATE
           FROM SOBPTRM
          WHERE SOBPTRM_TERM_CODE = term_in
            AND SOBPTRM_PTRM_CODE = ptrm_in;
...
-- Call local               bwckregs.p_final_updates (
            ...
            --
            -- C3SC 8.9.1.1 GRS 07/03/2014 ADDED
              FUNCTION f_is_crs_calc_drop_valid( p_pidm_in       IN   SPRIDEN.SPRIDEN_PIDM%TYPE,
                                                   p_rsts_code_in  IN   SFRRSTS.SFRRSTS_RSTS_CODE%TYPE,
                                                   p_term_code_in  IN  STVTERM.STVTERM_CODE%TYPE,
                                                   p_crn_code_in   IN  SSBSECT.SSBSECT_CRN%TYPE,
                                                   p_part_term_code_in IN SSBSECT.SSBSECT_PTRM_CODE%TYPE DEFAULT NULL,
                                                   p_registration_date_in IN ssbsect.ssbsect_reg_to_date%TYPE DEFAULT NULL
              )
                 RETURN BOOLEAN

                 IS
               CURSOR cur_get_part_of_term
                   IS
                     SELECT SSBSECT_PTRM_CODE
                      FROM SSBSECT
                      WHERE SSBSECT_TERM_CODE = p_term_code_in
                        AND SSBSECT_CRN       = p_crn_code_in;

                 regstatus_date_in DATE;
                 v_part_term_code_in  STVTERM.STVTERM_CODE%TYPE;
                 open_learning BOOLEAN := FALSE;
             gtvsdax_row           gtvsdax%ROWTYPE;
              calc_drop_code        stvrsts.stvrsts_code%TYPE;

             BEGIN
                   IF p_registration_date_in IS NOT NULL THEN
                     regstatus_date_in := p_registration_date_in;
                   ELSE
                     regstatus_date_in := trunc(SYSDATE);
                   END IF;


                   IF p_part_term_code_in IS NULL THEN
                         OPEN cur_get_part_of_term;
                         FETCH cur_get_part_of_term INTO v_part_term_code_in;
                         CLOSE cur_get_part_of_term;
                       ELSE
                        v_part_term_code_in := p_part_term_code_in;
                       END IF;

                       -- IF v_part_term_code_in IS NOT FOUND THEN IT IS A OPEN LEARNING COURSE
                       -- CHECK FOR OPEN LEARNING COURSE
                       -----------------------------------
                       IF v_part_term_code_in IS NULL THEN     -- OL COURSE
                    open_learning := TRUE;
                       END IF;

                   IF NOT open_learning
                       THEN
                         -- C3SC Defect 1-3JBJFC   GRS 04/10/2008
                         IF sv_acad_calendar_bp.f_is_waitlisted (p_rsts=> p_rsts_code_in) = 'Y' THEN
                            sfkcurs.p_get_gtvsdax ('WEBRSTSDRP', 'WEBREG', gtvsdax_row);
                            calc_drop_code := gtvsdax_row.gtvsdax_external_code;
                         ELSE
-- Remove twbkwbis always will be a student
--                           IF NVL (twbkwbis.f_getparam (p_pidm_in, 'STUFAC_IND'), 'STU') = 'FAC' THEN
--                               calc_drop_code:= sv_acad_calendar_bp.F_Calc_Fac_Drop_Code (p_term_code_in,
--                                                                             p_crn_code_in,
--                                                                             regstatus_date_in);
--                           ELSE
                               calc_drop_code:= sv_acad_calendar_bp.F_Calc_Drop_Code (p_term_code_in,
                                                                                  p_crn_code_in,
                                                                                 regstatus_date_in);
--                           END IF;
                          /* C3SC END */
                   ...
                   IF  NOT sv_auth_addcodes_bp.f_is_valid_rsts_code (p_rsts_code  => p_rsts_code_in)
                        AND calc_drop_code is NOT NULL
                AND calc_drop_code <> p_rsts_code_in
                   THEN
                      RETURN FALSE;
                END IF;
                 RETURN TRUE;
             END f_is_crs_calc_drop_valid;
""";

        def plsql5 = """
--BWCKLIBS.f_is_crs_reg_status_valid rewritten.
            FUNCTION f_is_crs_reg_status_valid (
                p_rsts_code_in  IN  SFRRSTS.SFRRSTS_RSTS_CODE%TYPE,
                p_term_code_in  IN  STVTERM.STVTERM_CODE%TYPE,
                p_crn_code_in   IN  SSBSECT.SSBSECT_CRN%TYPE,
                p_part_term_code_in IN SSBSECT.SSBSECT_PTRM_CODE%TYPE DEFAULT NULL
            ...
            RETURN BOOLEAN
            /*
            This function checks following two validations
            (1) It checks if the registration status is valid for web registration or not.
            (2) It checks if the registration status is valid for date range specified in SFRRSTS.
            */
            IS

                CURSOR cur_check_rsts_valid_for_web
                IS
                SELECT 'Y'
                  FROM stvrsts
                 WHERE stvrsts_code     = p_rsts_code_in
-- Enable non-web registration codes for
                ;
--                   AND stvrsts_web_ind  = 'Y';
                ...
                CURSOR cur_chk_rsts_valid_for_course(p_cur_part_term_code_in SSBSECT.SSBSECT_PTRM_CODE%TYPE)
                IS
                SELECT 'Y'
                  FROM SFRRSTS
                 WHERE (
                         TRUNC(SYSDATE) BETWEEN TRUNC(SFRRSTS_START_DATE) AND TRUNC(SFRRSTS_END_DATE)
                       )
                   AND SFRRSTS_RSTS_CODE = p_rsts_code_in
                   AND SFRRSTS_PTRM_CODE = p_cur_part_term_code_in
                   AND SFRRSTS_TERM_CODE = p_term_code_in;

                CURSOR cur_chk_rsts_valid_for_OLR
                IS
                SELECT 'Y' ,
                       /* SB 8.7.1.1 changes start */
                       ssrrsts_usage_cutoff_pct_from,
                       ssrrsts_usage_cutoff_pct_to,
                       ssrrsts_usage_cutoff_dur_from,
                       ssrrsts_usage_cutoff_dur_to
                        /* SB 8.7.1.1 changes end */
                  FROM SSRRSTS
                 WHERE SSRRSTS_RSTS_CODE    = p_rsts_code_in
                   AND SSRRSTS_CRN          = p_crn_code_in
                   AND SSRRSTS_TERM_CODE    = p_term_code_in;

                v_valid_yes_no      VARCHAR2(1) := NULL;
                v_part_term_code_in SSBSECT.SSBSECT_PTRM_CODE%TYPE := NULL;

                /* SB 8.7.1.1 changes start */
                v_sftregs_start_date sftregs.sftregs_start_date%TYPE;
                v_sftregs_comp_date sftregs.sftregs_completion_date%TYPE;
                v_sftregs_dunt_code sftregs.sftregs_dunt_code%TYPE;
                v_ssrrsts_cutoff_pct_f ssrrsts.ssrrsts_usage_cutoff_pct_from%TYPE;
                v_ssrrsts_cutoff_pct_t ssrrsts.ssrrsts_usage_cutoff_pct_to%TYPE;
                v_ssrrsts_cutoff_dur_f ssrrsts.ssrrsts_usage_cutoff_dur_from%TYPE;
                v_ssrrsts_cutoff_dur_t ssrrsts.ssrrsts_usage_cutoff_dur_to%TYPE;
                /* SB 8.7.1.1 changes end */

            BEGIN
                OPEN cur_check_rsts_valid_for_web;
                FETCH cur_check_rsts_valid_for_web INTO v_valid_yes_no;

                IF cur_check_rsts_valid_for_web%NOTFOUND
                THEN
                  CLOSE cur_check_rsts_valid_for_web;
                  RETURN FALSE;
                END IF;
                CLOSE cur_check_rsts_valid_for_web;
                -----------------------------------
                ...
                -- CHECK FOR OPEN LEARNING COURSE
                -----------------------------------
                IF v_part_term_code_in IS NOT NULL THEN     -- REGULAR COURSE
                    OPEN cur_chk_rsts_valid_for_course(v_part_term_code_in);
                    FETCH cur_chk_rsts_valid_for_course INTO v_valid_yes_no;

                    IF cur_chk_rsts_valid_for_course%NOTFOUND
                    THEN
                      CLOSE cur_chk_rsts_valid_for_course;
                      RETURN FALSE;
                    END IF;
                    CLOSE cur_chk_rsts_valid_for_course;
                ELSE    -- OLR COURSE
                    ...
                    FOR sftregs_row IN (SELECT sftregs_start_date,
                           sftregs_completion_date, sftregs_dunt_code
                       FROM sftregs
-- Call package                      WHERE sftregs_pidm = f_getpidm
                      WHERE sftregs_pidm = bwcklibs.f_getpidm
                        AND sftregs_term_code = p_term_code_in
                        AND sftregs_crn = p_crn_code_in )

                    LOOP
                          v_sftregs_start_date   := sftregs_row.sftregs_start_date;
                          v_sftregs_comp_date    := sftregs_row.sftregs_completion_date;
                          v_sftregs_dunt_code    := sftregs_row.sftregs_dunt_code;

                    END LOOP;
                    /* SB 8.7.1.1 changes end */

                    OPEN cur_chk_rsts_valid_for_OLR;
                    /* SB 8.7.1.1 changes. Except v_valid_yes_no, all are new variables */
                    FETCH cur_chk_rsts_valid_for_OLR INTO v_valid_yes_no,v_ssrrsts_cutoff_pct_f,
                               v_ssrrsts_cutoff_pct_t,v_ssrrsts_cutoff_dur_f,v_ssrrsts_cutoff_dur_t;

                    /* SB 8.7.1.1 changes. added OR sfkolrl.f_cutoff_reached condition */
                    IF cur_chk_rsts_valid_for_OLR%NOTFOUND OR sfkolrl.f_cutoff_reached (
                              v_sftregs_start_date,
                              v_sftregs_comp_date,
                              trunc(SYSDATE),
                              v_sftregs_dunt_code,
                              v_ssrrsts_cutoff_pct_f,
                              v_ssrrsts_cutoff_pct_t,
                              v_ssrrsts_cutoff_dur_f,
                              v_ssrrsts_cutoff_dur_t)
                    THEN
                      CLOSE cur_chk_rsts_valid_for_OLR;
                      RETURN FALSE;
                    END IF;
                    CLOSE cur_chk_rsts_valid_for_OLR;
...
--                     twbkwbis.p_closedoc (curr_release);
              ...
              LOOP
                    ...
                    THEN
-- Call local                        IF NOT bwcklibs.f_is_crs_reg_status_valid( rsts_in (i), assoc_term (i), crn (i) ) THEN
                        IF NOT f_is_crs_reg_status_valid( rsts_in (i), assoc_term (i), crn (i) ) THEN
-- Raise error                          twbkwbis.p_dispinfo (called_by_proc_name, 'CRSREGSTATUSNOTVALID');
                            raise_application_error(-20001, 'Registration status code configured in adaptor is not valid');
                          RETURN;
...
-- Call local                                      IF NOT sv_acad_calendar_bp.f_is_crs_calc_drop_valid(p_pidm_in=>genpidm,
                                      IF NOT f_is_crs_calc_drop_valid(p_pidm_in=>genpidm,
