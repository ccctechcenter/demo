...
class SharedApplication {
    ...
    static Map<String, MetaProperty> appProperties = com.ccctc.adaptor.model.apply.SharedApplication.metaClass.properties.collectEntries { [(it.name.toUpperCase()): it] }

    def get(String misCode, Long id) {
        Sql sql = BannerConnection.getSession(environment, misCode)

        try {
            def applicationQuery = MisEnvironment.getProperty(environment, misCode, "sqljdbc.application.shared.applicationQuery")

            GroovyRowResult applicationRec = sql.firstRow(applicationQuery, [appId: id, misCode:misCode])

            if (!applicationRec)
                throw new EntityNotFoundException()


            def returnApplication = new com.ccctc.adaptor.model.apply.SharedApplication()
            Utility.mapValues(appProperties, applicationRec, returnApplication)
    ...
    def post(String misCode, Application application) {
        ...
        try {
            ...
            def applicationrec = sql.firstRow(applicationQuery, [appId: application.getAppId(), misCode: misCode])
            if (applicationrec)
                throw new EntityConflictException("Application ID already exists for given MIS")
    ...
    def createApply(String misCode, def application, Sql sql) {
       def insertQuery = MisEnvironment.getProperty(environment, misCode, "sqljdbc.application.shared.insertQuery")
        try {
            sql.execute(insertQuery, ["misCode": misCode,
