...
/**
 * <h1>CCCApply Standard Application Colleague Interface</h1>
 * <summary>
 *     <p>Provides:</p>
 *     <ol>
 *         <li>Storing and populating Standard Applications from CCCApply into a single custom colleague staging table</li>
 *         <li>Retrieving Standard Applications from the custom colleague staging table</li>
 *     <ol>
...
class Apply {
    ...
    Application get(String misCode, Long appId) {
        ...
        EntityMetadata entity = entityMetadataService.get("ST", "XCTC.CCC.APPLY.APPL")

        if(entity == null) {
            String errMsg = "get: unable to retrieve application metadata"
            log.error(errMsg)
        ...
        ColleagueData application = dmiDataService.singleKey("ST", "XCTC.CCC.APPLY.APPL", columns, appId.toString())
        ...
        Application result = this.mapFromColleague(application)
    ...
    /**
     * Sets Standard Application Data into a Colleague staging table using the DMI Client
     * @param misCode The College Code used for grabbing college specific settings from the environment
     * @param application The application data to set into the staging table
     * @returns void/null
     */
    void post(String misCode, Application application) {
        log.debug("post: setting standard application data")

        //****** Validate parameters ****
        if (application == null) {
            String errMsg = "post: Standard Application data cannot be null"
            log.warn(errMsg)
            throw new InvalidRequestException(InvalidRequestException.Errors.invalidRequest, errMsg)
        }
        ...
        ColleagueData existing_application = dmiDataService.singleKey("ST", "XCTC.CCC.APPLY.APPL", [ this.toColleagueField("cccId", false) ], application.appId.toString())
        if (existing_application != null && existing_application.key == application.appId.toString()) {
            String errMsg = "post: Application ID already exists"
            log.warn(errMsg)
        ...
        DataWriteResult app_writeResult = dataWriter.write("XCTC.CCC.APPLY.APPL", application.appId.toString(), false, record.toList())
     ...
     */
    private Application mapFromColleague(ColleagueData data) {
        Application result = new Application()
        result.supplementalQuestions = new SupplementalQuestions()
        ...
        // map supplemental questions
        boolean hasSupplementalValues = false
        result.supplementalQuestions.metaClass.properties.each { p ->
            String javaField = p.name
            Object value = null
            ...
            if (value != null) {
                result.supplementalQuestions."$javaField" = value
                hasSupplementalValues = true
            }
        }

        // remove supplemental questions if there are no values
        if (!hasSupplementalValues) {
            result.supplementalQuestions = null
        } else {
            // copy app id to supplemental questions
     ...
     */
    private String[] mapToColleague(Application application) {

        EntityMetadata entity = entityMetadataService.get("ST", "XCTC.CCC.APPLY.APPL")

        if(entity == null) {
            String errMsg = "mapToColleague: Unable to retrieve metaData columns for Standard Application"
            log.error(errMsg)
            throw new InternalServerException(errMsg)
        }
        ...
        if(recordSize == 0) {
            String errMsg = "mapToColleague: Unable to determine recordSize for Standard Application mapping"
            log.error(errMsg)
            throw new InternalServerException(errMsg)
        }
        ...
        if (application.supplementalQuestions != null) {
            application.supplementalQuestions.properties.each { p ->
                if (p.value != null) {
                    CddEntry cdd = this.getCddEntry(entity, (String) p.key)
                    this.mapValue(p.value, cdd, record)
                }
    ...
    /**
     * Convert a java field to a colleague field name in the XCTC.CCC.APPLY.APPL table.
     * Camel case is converted to upper case with "." delimiting words. So collegeId becomes XCTC.CA.COLLEGE.ID.
     * <p>
    ...
    private String toColleagueField(String javaFieldName, boolean numberMode) {
        ...
        switch (javaFieldName) {
            case "emailVerifiedTimestampDate":
                return "XCTC.CA.EMAIL.VERIFIED.DATE"
            case "emailVerifiedTimestampTime":
                return "XCTC.CA.EMAIL.VERIFIED.TIME"
            case "mainphoneVerifiedTimestampDate":
                return "XCTC.CA.MAINPHONE.VRFD.DATE"
            case "mainphoneVerifiedTimestampTime":
                return "XCTC.CA.MAINPHONE.VRFD.TIME"
            case "addressValidationOverrideTimestampDate":
                return "XCTC.CA.ADDRESS.VALID.O.DATE"
            case "addressValidationOverrideTimestampTime":
                return "XCTC.CA.ADDRESS.VALID.O.TIME"
            case "preferredMethodOfContact":
                return "XCTC.CA.PREFERRED.METHOD.CON"
            case "highestMathCourseTaken":
                return "XCTC.CA.HIGHEST.MATH.CRS.TAK"
            case "highestMathCoursePassed":
                return "XCTC.CA.HIGHEST.MATH.CRS.PAS"
            case "highestMathPassedGrade":
                return "XCTC.CA.HIGHEST.MATH.PASSEDG"
            case "acceptedTerms":
                return "XCTC.CA.ACCEPTED.TERMS";
            case "acceptedTermsTimestampDate":
                return "XCTC.CA.ACCEPTED.TERMS.DATE";
            case "acceptedTermsTimestampTime":
                return "XCTC.CA.ACCEPTED.TERMS.TIME";
            case "mailingAddressValidationOverride":
                return "XCTC.CA.MA.ADDRESS.VALID.O";
            case "phoneType":
                return "XCTC.CA.MAINPHONE.TYPE";
            case "ipAddressAtAccountCreation":
                return "XCTC.CA.IP.ADDRESS.ACCT.CRTD";
            case "ipAddressAtAppCreation":
                return "XCTC.CA.IP.ADDRESS.APPL.CRTD";
            case "studentParent":
                return "XCTC.CA.STUDENT.PARENT";
            case "studentDepsUnder18":
                return "XCTC.CA.STUDENT.DEPS.UNDER18";
            case "studentDeps18Over":
                return "XCTC.CA.STUDENT.DEPS.18OVER";
            case "idmeConfirmationTimestampDate":
                return "XCTC.CA.IDME.CONFIRMED.DATE";
            case "idmeConfirmationTimestampTime":
                return "XCTC.CA.IDME.CONFIRMED.TIME";
            case "idmeOptinTimestampDate":
                return "XCTC.CA.IDME.OPTIN.DATE";
            case "idmeOptinTimestampTime":
                return "XCTC.CA.IDME.OPTIN.TIME";
            case "idmeWorkflowStatus":
                return "XCTC.CA.IDME.WORKFLOW.STATUS";
            case "raceAIANOtherDescription":
                return "XCTC.CA.RACE.AIAN.OTHER.DESC";
            case "sisProcessedFlag":
                //For sisProcessedFlag we get the value of processed date and later checks if there is a value
                return "XCTC.CA.PROCESSED.DATE"
            case "tstmpSISProcessedDate":
                return "XCTC.CA.PROCESSED.DATE"
            case "tstmpSISProcessedTime":
                return "XCTC.CA.PROCESSED.TIME"
            case "sisProcessedNotes":
                return "XCTC.CA.USER1"
        }
        ...
        String fieldName = "XCTC.CA." + new String(newChars, 0, pos)
