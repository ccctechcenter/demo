...
class CCPromiseGrant {
    ...
    /**
     * Gets CCPromise Grant Application Data from a Colleague staging table using the DMI Client
     ...
     * @returns A matching CCPromise Grant Application
     */
    CCPromiseGrantAppl get(String misCode, Long appId) {
        ...
        EntityMetadata entity = entityMetadataService.get("ST", "XCTC.CCPG.APPL")

        if(entity == null) {
            String errMsg = "get: unable to retrieve ccpg application metadata"
            log.error(errMsg)
        ...
        ColleagueData application = dmiDataService.singleKey("ST", "XCTC.CCPG.APPL", columns, appId.toString())

        if (application == null) {
            String errMsg = "get: CC Promise Grant application not found"
            log.warn(errMsg)
            throw new EntityNotFoundException(errMsg)
        }

        log.debug("get: app found; mapping fields")
        CCPromiseGrantAppl result = this.mapFromColleague(application)
    ...
    /**
     * Sets CCPromise Grant Application Data into a Colleague staging table using the DMI Client (Through DataWriter)
     * @param misCode The College Code used for grabbing college specific settings from the environment
     * @param application The application data to set into the staging table
     */
    void post(String misCode, CCPromiseGrantAppl application) {
        log.debug("post: setting ccpromise grant application data")

        //****** Validate parameters ****
        if (application == null) {
            String errMsg = "post: CCPG Application data cannot be null"
            log.warn(errMsg)
            throw new InvalidRequestException(InvalidRequestException.Errors.invalidRequest, errMsg)
        }
        ...
        ColleagueData existing_application = dmiDataService.singleKey("ST", "XCTC.CCPG.APPL", [ this.toColleagueField("cccId", false) ], application.appId.toString())
        if (existing_application != null && existing_application.key == application.appId.toString()) {
            String errMsg = "post: CCPG Application ID already exists"
            log.warn(errMsg)
        ...
        DataWriteResult app_writeResult = dataWriter.write("XCTC.CCPG.APPL", application.appId.toString(), false, record.toList())
    ...
    /**
     * Map data from Colleague to a CC Promise Grant application
     */
    private CCPromiseGrantAppl mapFromColleague(ColleagueData data) {
        CCPromiseGrantAppl result = new CCPromiseGrantAppl()
    ...
    /**
     * Map data from an CC Promise Grant application to a String array for transmission to Colleague
     */
    private String[] mapToColleague(CCPromiseGrantAppl application) {

        EntityMetadata entity = entityMetadataService.get("ST", "XCTC.CCPG.APPL")

        if(entity == null) {
            String errMsg = "mapToColleague: Unable to retrieve metaData columns for ccpg Application"
            log.error(errMsg)
            throw new InternalServerException(errMsg)
        }
        ...
        if(recordSize == 0) {
            String errMsg = "mapToColleague: Unable to determine recordSize for ccpg Application mapping"
            log.error(errMsg)
            throw new InternalServerException(errMsg)
        }
    ...
    /**
     * Convert a java field to a colleague field name in the XCTC.CCPG.APPL table.
     * Camel case is converted to upper case with "." delimiting words. So collegeId becomes XCTC.CP.COLLEGE.ID.
     * <p>
    ...
    private String toColleagueField(String javaFieldName, boolean numberMode) {
        ...
        switch (javaFieldName) {
            case "emailVerifiedTimestampDate":
                return "XCTC.CP.EMAIL.VERIFIED.DATE"
            case "emailVerifiedTimestampTime":
                return "XCTC.CP.EMAIL.VERIFIED.TIME"
            case "mainphoneVerifiedTimestampDate":
                return "XCTC.CP.MAINPHONE.VRFD.DATE"
            case "mainphoneVerifiedTimestampTime":
                return "XCTC.CP.MAINPHONE.VRFD.TIME"
            case "addressValidationOverrideTimestampDate":
                return "XCTC.CP.ADDRESS.VALID.DATE"
            case "addressValidationOverrideTimestampTime":
                return "XCTC.CP.ADDRESS.VALID.TIME"
            case "preferredMethodOfContact":
                return "XCTC.CP.PREFERRED.METHOD.CON"
            case "dependentOnParentTaxes":
                return "XCTC.CP.DEPENDENT.PARENT.TAX"
            case "confirmationParentGuardian":
                return "XCTC.CP.CONFIRMATION.PARENTG"
            case "determinedAB540Eligible":
                return "XCTC.CP.DETERMINED.AB540.ELI"
            case "acceptedTerms":
                return "XCTC.CP.ACCEPTED.TRMS"
            case "acceptedTermsTimestampDate":
                return "XCTC.CP.ACCEPTED.TRMS.DATE"
            case "acceptedTermsTimestampTime":
                return "XCTC.CP.ACCEPTED.TRMS.TIME"
            case "phoneType":
                return "XCTC.CP.MAINPHONE.TYPE"
            case "mailingAddressValidationOverride":
                return "XCTC.CP.ADDRESS.VALID.OVERD"
            case "ipAddressAtAccountCreation":
                return "XCTC.CP.IP.ADDRESS.ACCT"
            case "ipAddressAtAppCreation":
                return "XCTC.CP.IP.ADDRESS.APPL"
            case "studentParent":
                return "XCTC.CP.STUDENT.PARENT"
            case "studentDepsUnder18":
                return "XCTC.CP.STUDENT.DEPS.UNDER18"
            case "studentDeps18Over":
                return "XCTC.CP.STUDENT.DEPS.18OVER"
            case "idmeConfirmationTimestampDate":
                return "XCTC.CP.IDME.CONFIRMED.DATE"
            case "idmeConfirmationTimestampTime":
                return "XCTC.CP.IDME.CONFIRMED.TIME"
            case "idmeOptinTimestampDate":
                return "XCTC.CP.IDME.OPTIN.DATE"
            case "idmeOptinTimestampTime":
                return "XCTC.CP.IDME.OPTIN.TIME"
            case "idmeWorkflowStatus":
                return "XCTC.CP.IDME.WORKFLOW.STATUS";
            case "sisProcessedFlag":
                //Gets the value of processed date and later checks if there is a value
                return "XCTC.CP.PROCESSED.DATE"
            case "tstmpSISProcessedDate":
                return "XCTC.CP.PROCESSED.DATE"
            case "tstmpSISProcessedTime":
                return "XCTC.CP.PROCESSED.TIME"
            case "sisProcessedNotes":
                return "XCTC.CP.USER1"
        }
        ...
        String fieldName = "XCTC.CP." + new String(newChars, 0, pos)
